...
import { DollarSign, ChevronsDownUp, ChevronsUpDown } from 'lucide-react-native';
...
export default function BatchExpensesTab({ batchId }) {
  ...
  const totalSpend = useMemo(() => filteredExpenses.reduce((s, x) => s + (x.totalAmount || 0), 0), [filteredExpenses]);
  ...
  return (
    <View className="flex-1 bg-background">
      <View className="px-4 pt-3 pb-2 flex-row gap-2">
        <SummaryChip label={t('batches.entries', 'Entries')} value={expenses.length} />
        <SummaryChip label={t('batches.totalSpend', 'Total Spend')} value={fmt(totalSpend)} />
      </View>
